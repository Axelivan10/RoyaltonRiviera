...
import Form_plant from '../general.components/form_plant';
import { Card, Typography } from '@material-tailwind/react';
import { getDepartment, getDivision, getHotels, getLocation, getLocationConfig, getPlantConfig, getRelationsDepartment, getRelationsLocation, relationsLocationConfig, relationsPlantConfig, updatePlantConfig } from '../../api/manning.api';
import { Cog6ToothIcon, TableCellsIcon } from '@heroicons/react/24/outline';
import TableCurrentPlant from '../general.components/table_CurrentPlant';
...
function plant() {
    ...
    const renderInputs = async ()  =>{
      ...
      plantConfig.data.forEach((config:any) => {
        if (config.xSymbol) {
  ...
  const updateData = () => {
    try {
      updatePlantConfig(valuesToSend);
    } catch (error) {
