...
import { getDepartment, getDivision, getHotels, getLocation, getRelationsPositionConfig, getShift, getShiftConfig, relationsShiftConfig, updatePositionConfig, updateShiftConfig } from '../../api/manning.api';
...
function position() {
    ...
    const [relationsPosition, setRelationsPosition] = useState<relationsAll[]>([]);
    ...
    useEffect(()=>{
      ...
      const renderList = async () => {
        try {
            ...
            const responseLocations = await getLocation();
            setLocations(responseLocations.data)
            console.log(responseLocations.data  )

            const responseRelationsPositionConfig = await getRelationsPositionConfig();
            setRelationsPosition(responseRelationsPositionConfig.data)
            console.log(responseRelationsPositionConfig.data)
      ...
      const handleInputChange = ( positionId: number, locationId: number, value: string ) => {
          if (value.toLowerCase() === "x" || value.toLowerCase() === "") {
            const key = `${positionId}-${locationId}`;
      ...
      const handleDivisionChange = (e: any) => {
        ...
        const renderInputs = async ()  =>{
          const positionConfig = await getRelationsPositionConfig();
          console.log(positionConfig.data)
          positionConfig.data.forEach((config:any) => {
            if (config.xSymbol) {
        ...
        const updateData = () => {
        try {
          updatePositionConfig(valuesToSend);
        } catch (error) {
      ...
      <div className="flex flex-col-1 gap-8 ml-auto pr-4 pt-0.5 justify-end p-4">
        ...
        >
          <table className="w-full min-w-max table-auto text-left ">
            ...
            <tbody>
              {filteredData.map(
                ({ id: positionId, positiondim, deparment, location, plant }) => (
                  <tr key={positionId}>
                    ...
                    {locations.map(({ id: LocationId, areaCode }) => (
                      ...
                      >
                        <input
                          ...
                          value={inputValues[`${positionId}-${LocationId}`] || ""}
                          onChange={(e) =>
                            handleInputChange(positionId, LocationId, e.target.value)
                          }
                          disabled={!isActive}
                        />
                      </td>
