...
import { getDepartment, getDivision, getHotels, getRelationsKitchenBackConfig, updateKitchenBackConfig,  } from '../../api/manning.api';
import Swal from 'sweetalert2';

const TABLE_HEAD = ["Service Type", "Shift Code" ,"Position Id", "Position Description"];
...
function kitchenBack() {
  ...
  const [relationsKitchenBack, setRelationsKitchenBack] = useState<relationsAll[]>([]);
  ...
  useEffect(() => {
   ...
   useEffect(() => {
     ...
     const relacionesUnicas = eliminarDuplicados(relationsKitchenBack);
     setNoDuplicates(relacionesUnicas);
   }, [relationsKitchenBack]);
  ...
  const eliminarDuplicados = (relationsPlant: relationsAll[]) => {
    ...
    relationsPlant.forEach(entry => {
      const key = `${entry.serviceType.serviceTypeId}-${entry.shift.shiftId}-${entry.positiondim.positionId}`;
      ...
      if (!uniqueEntries.has(key)) {
        ...
        result.push({
          ...
          positiondim: {
            positionId: entry.positiondim.positionId,
  ...
  const renderList = async () => {
    try {
        ...
        const responseRelationsKitchenBackConfig = await getRelationsKitchenBackConfig();
        setRelationsKitchenBack(responseRelationsKitchenBackConfig.data)
        console.log(responseRelationsKitchenBackConfig.data)  
  ...
  const handleInputChange = ( tableId: number, plantId: number, value: string ) => {
    if (/^[0-9 ]*$/.test(value) || value.toLowerCase() === "") {
  ...
  const renderInputs = async ()  =>{
    const KitchenBackConfig = await getRelationsKitchenBackConfig();

    KitchenBackConfig.data.forEach((config:any) => {
  ...
  const updateData = () => {
  try {
    updateKitchenBackConfig(valuesToSend);
  } catch (error) {
     ...
     <div className="flex flex-col-1 gap-8 ml-auto pr-4 pt-0.5 justify-end p-4 pb-4">
      ...
      >
        <table className="w-full min-w-max table-auto text-left ">
          ...
          <tbody>
            {noDupicates.map(
              ({ id: tableId, serviceType, shift, positiondim }) => (
                <tr key={tableId}>
